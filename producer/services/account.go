...
func (service accountService) OpenAccount(command commands.OpenAccountCommand) (id string, err error) {
	...
	log.Printf("%#v", event)
	return event.ID, service.eventProducer.Produce(event)
}

func (service accountService) DepositFund(command commands.DepositFundCommand) error {
	...
	log.Printf("%#v", event)
	return service.eventProducer.Produce(event)
}

func (service accountService) WithdrawFund(command commands.WithdrawFundCommand) error {
	if command.ID == "" || command.Amount == 0 {
		return errors.New("bad request")
	}
...
func (service accountService) CloseAccount(command commands.CloseAccountCommand) error {
	if command.ID == "" {
		return errors.New("bad request")
	}
